/**
 * Informational report (not a gate): correlation between the playstyle Axes across reviewed Builds,
 ...
 */
import { PLAYSTYLE_AXIS_IDS, normalize, type PlaystyleAxisId } from '../src/lib/axes'
...
console.log(pad('') + PLAYSTYLE_AXIS_IDS.map((a) => pad(a)).join(''))
for (const a of PLAYSTYLE_AXIS_IDS) {
  console.log(pad(a) + PLAYSTYLE_AXIS_IDS.map((b) => pad(pearson(columns[a], columns[b]).toFixed(2))).join(''))
}
const strong = PLAYSTYLE_AXIS_IDS.flatMap((a, i) =>
  PLAYSTYLE_AXIS_IDS.slice(i + 1)
