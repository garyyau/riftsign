...
export function RankingList({ matches }: { matches: Match[] }) {
  return (
    <Collapsible className="border-t">
      <CollapsibleTrigger className="small-caps group flex w-full cursor-pointer items-center justify-between py-5 text-left text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <ol className="divide-y border-t">
          {matches.map((m, i) => (
            <li key={m.legend.id} className="grid grid-cols-[2.5rem_1fr_auto] items-center gap-3 py-3 text-sm">
              <span className="text-muted-foreground tabular-nums">{stepNumber(i + 1)}</span>
              <span className="min-w-0">
                <span className="block truncate">{m.legend.name}</span>
                <span className="mt-1 flex flex-wrap gap-3">
                  <span className="small-caps text-muted-foreground">{m.build.archetype}</span>
                  {m.legend.domains.map((d) => (
                    <DomainTag key={d} domain={d} />
                  ))}
                </span>
              </span>
              <span className="tabular-nums">{m.fit}%</span>
            </li>
          ))}
        </ol>
      </CollapsibleContent>
